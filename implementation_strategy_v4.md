...
You have received several review_comments from {num_reviewers} reviewers, which are presented in the {review_comments} block.
...
The final manuscript must be **well-structured, correctly formatted, and scientifically rigorous**, thereby enhancing both readability and impact. It should be written in fluent academic {manuscript_language}.
...
- Incorporate tags like `\revtag{1}{1} \Rone{Some text}` before every modified piece of text, where the first attribute {1} represents Reviewer 1, and the second attribute {1} stands for Comment 1 of Reviewer 1, which is being addressed in the corresponding piece of text. Utilise the following revision markup commands:
...
- Provide your explanations on the committed modifications in the Markdown format in a separate `.md` block.

- Utilise as much time and as many tokens as possible to ensure the best answer possible.
...
{num_reviewers} = “two”
{project_directory} = “transition-matrix-dss/manuscript/”
...
Produce **fully formatted, error-free, submission-ready files**: (i) the revised {manuscript_tex} block in the `main.tex` file, (ii) the revised {refs_bibtex} block in the `references.bib` file, (iii) the revised {supply_tex} block in `supply.tex` file, and (iv) your explanations on the provided modifications in the Markdown format in the newly created `my_revision.md` file.
