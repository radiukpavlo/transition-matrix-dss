...
class NatureFigureGenerator:
    ...
    def fig01_framework(self) -> None:
        ...
        for i, ((title, sub), x) in enumerate(zip(steps, xs)):
            ...
            ax.add_patch(box)
            ax.text(x, 0.59, title, ha="center", va="center", fontsize=6.6, color=contrast_text_color(fc), fontweight="bold")
            ax.text(x, 0.52, sub, ha="center", va="center", fontsize=5.8, color=contrast_text_color(fc))
            if i < len(xs) - 1:
                ax.annotate("", xy=(xs[i + 1] - 0.057, 0.57), xytext=(x + 0.057, 0.57), arrowprops=dict(arrowstyle="-|>", lw=0.65, color=PALETTE["dark"]))
        ax.text(0.5, 0.25, "Row alignment -> thresholds -> support/confidence -> conflicts -> abstention", ha="center", va="center", fontsize=6.8, color=PALETTE["black"])
        self.save(fig, "fig01_framework_pipeline.pdf", [], "schematic-led composite")
    ...
    def fig02_matrix_alignment(self) -> None:
        ...
        for label, dim, sub, x, color in dims:
            ...
            ax.text(x, 0.58, label, ha="center", va="center", fontsize=11.0, fontweight="bold", color=contrast_text_color(color))
            ax.text(x, 0.49, dim, ha="center", va="center", fontsize=6.6, color=contrast_text_color(color))
            ax.text(x, 0.30, sub, ha="center", va="center", fontsize=6.0, color=PALETTE["dark"])
        ax.text(0.295, 0.53, "x", ha="center", va="center", fontsize=13.0, color=PALETTE["dark"])
        ax.text(0.605, 0.53, "=", ha="center", va="center", fontsize=13.0, color=PALETTE["dark"])
        ax.text(0.5, 0.17, "Object identifiers and decision labels remain aligned through the transition pipeline.", ha="center", fontsize=6.4)
    ...
    def fig04_svd_variance(self) -> None:
        ...
        ax.text(n * 0.58, retained + 0.012, f"retained={retained:.3f}", fontsize=6.0, color=PALETTE["dark"])
    ...
    def fig15_coverage_tradeoff(self) -> None:
        ...
        ax.set_ylabel("Score")
        ax.set_ylim(0, 1.05)
        ax.legend(ncol=2, fontsize=5.8, loc="upper center", bbox_to_anchor=(0.5, -0.28))
        style_axis(ax, "y")
    ...
    def fig19_tradeoff(self) -> None:
        ...
        for offset, (_, row) in zip(offsets, df.iterrows()):
            ax.annotate(clean_label(format_method(row["method"]), 14), (row["coverage"], row["accuracy_covered"]), xytext=offset, textcoords="offset points", fontsize=6.0, arrowprops=dict(arrowstyle="-", linewidth=0.45, color=PALETTE["mid"]))
    ...
    def fig20_heatmap(self) -> None:
        ...
        for i in range(data.shape[0]):
            for j in range(data.shape[1]):
                ax.text(j, i, f"{data[i, j]:.2f}", ha="center", va="center", fontsize=6.0, color=PALETTE["black"])
    ...
    def fig21_funnel(self) -> None:
        ...
        annotate_bars(ax, bars, values, fmt="{:.0f}", fontsize=5.8)
        style_axis(ax, "y")
    ...
    def fig24_salience_error(self) -> None:
        ...
        for offset, (_, row) in zip(offsets, top.iterrows()):
            ax.annotate(clean_label(row["attribute"], 10), (row["test_mae"], row["salience"]), xytext=offset, textcoords="offset points", fontsize=5.8, arrowprops=dict(arrowstyle="-", linewidth=0.4, color=PALETTE["mid"]))
    ...
    def run(self) -> None:
        ...
        manifest = {
            ...
            "dpi": self.dpi,
            "style": {
                "font_family": "Palatino Linotype / Palatino serif fallback",
                "pdf_fonttype": 42,
                "svg_fonttype": "none",
                "text_size_pt": "manuscript-fit Nature pass: 5.8-7.5 base, schematic emphasis up to 13",
            ...
            "notes": [
                ...
                "SVG and TIFF companion exports use matching stems.",
...
def main() -> None:
    ...
    parser.add_argument("--formats", nargs="+", default=["pdf", "svg", "tiff"], choices=["pdf", "svg", "tiff", "tif", "png"])
